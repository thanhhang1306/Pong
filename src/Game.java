...
public class Game  extends JPanel implements Runnable, KeyListener{ // KeyListener is a one way interface - registered when a key is press 
	...
	public Game() {
		back = null;
		new Thread(this).start();	// start the program; allow paint method to run in a loop
		this.addKeyListener(this); 	// start KeyListener 
	...
	public void paint(Graphics g){
		...
		if(activateStart){
			...
			move();
			Color color3 = new Color(225, 127, 147);
			g2d.setColor(color3);
	...
	@Override
	public void keyTyped(KeyEvent e) { // does not always work all the time 
	...
	public void keyPressed(KeyEvent e) { // Key is pressed, pass through e. 
		...
		key= e.getKeyCode(); // set integer to key code 
		System.out.println(key); 

		if(key==87){
			playerOne.setdy(-3);
			//  playerOne.move(-2)  // would not work because there is an infinite loop; this will only move it once
		}
		if(key==83){
			playerOne.setdy(3);
		}
